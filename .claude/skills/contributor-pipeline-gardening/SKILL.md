...
**Don't just aim for the floor (50) — push toward the top of the range (closer to 100) whenever real, non-padded gaps are still findable.** More well-scoped available issues is straightforwardly good for the project: it's more real work contributors can pick up. If the count is under ~100, keep sourcing issues until it's close to 100 (or a pass genuinely turns up no more real, non-duplicate gaps) — don't stop at a modest first batch, and don't declare victory the moment 50 is cleared. "Quality over volume" (point 6 below) means don't pad with weak/duplicate/vague issues, it does not mean stopping early once _a_ well-scoped batch is filed. When under target: extend an already-proven, well-precedented vein first if one exists (e.g. a REST/GraphQL/MCP parity sweep with many remaining candidates — the fastest path to more verified, non-padded issues), then dispatch parallel subsystem-audit agents across distinct areas of the codebase once that's exhausted — a single wave clearing 50 is a checkpoint, not a stopping point.
