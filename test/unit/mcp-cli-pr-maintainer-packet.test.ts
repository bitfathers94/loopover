...
async function connect() {
  configDir = mkdtempSync(join(tmpdir(), "loopover-pr-maintainer-packet-"));
  capturedRequests = [];
  ...
  client = new Client({ name: "pr-maintainer-packet-test", version: "0.0.1" });
...
describe("loopover_get_pr_maintainer_packet stdio proxy (#7802)", () => {
  beforeEach(connect);
  afterEach(disconnect);

  it("registers the tool in the stdio server tool list", async () => {
    const { tools } = await client.listTools();
    expect(tools.map((t) => t.name)).toContain("loopover_get_pr_maintainer_packet");
  });

  it("proxies owner/repo/number to /v1/repos/:owner/:repo/pulls/:number/maintainer-packet via apiGet", async () => {
