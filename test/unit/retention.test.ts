...
describe("pruneExpiredRecords", () => {
  ...
  it("dry-run falls back to 0 when the count query returns no row (defensive ?? 0 arm)", async () => {
    ...
    const results = await pruneExpiredRecords(noRowEnv, { dryRun: true, nowMs: NOW, policy: [{ table: "webhook_events", column: "received_at", days: 90 }] });
  ...
  it("falls back to 0 changes when a delete run() result lacks meta (defensive ?? 0 arm)", async () => {
    const noMetaEnv = {
      DB: {
        prepare: (_sql: string) => ({
          bind: (..._binds: unknown[]) => ({ run: async () => ({}) }), // no meta → `result.meta?.changes ?? 0` fallback fires, so changes = 0 < batchSize and the loop breaks
        }),
      },
    } as unknown as Env;
    const results = await pruneExpiredRecords(noMetaEnv, { nowMs: NOW, policy: [{ table: "webhook_events", column: "received_at", days: 90 }] });
    expect(results).toEqual([{ table: "webhook_events", column: "received_at", cutoff: daysAgo(90), deleted: 0 }]);
  });
});
