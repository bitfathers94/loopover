...
describe("planReplaySnapshotPath (#3010) — pure, deterministic", () => {
  ...
  it("rejects a path-traversal-shaped commitSha before it can escape the snapshot dir via path.join (#7796)", () => {
    expect(() =>
      planReplaySnapshotPath({ repoPath: "/home/miner/.config/loopover-miner/repos/acme/widgets", commitSha: "../../../../../../tmp/evil-worktree" }),
    ).toThrow("invalid_commit_sha");
  });

  it("rejects non-hex, too-short, blank, and over-long commitSha values (#7796)", () => {
    ...
    expect(() => planReplaySnapshotPath({ repoPath: "/repo", commitSha: "g".repeat(40) })).toThrow("invalid_commit_sha");
  });

  it("normalizes uppercase hex to a single canonical lowercase path (#7796)", () => {
    const lower = planReplaySnapshotPath({ repoPath: "/repo", commitSha: "abcdef0" });
    expect(planReplaySnapshotPath({ repoPath: "/repo", commitSha: "ABCDEF0" })).toBe(lower);
    expect(lower.replaceAll("\\", "/")).toBe(`/repo/${REPLAY_SNAPSHOT_SUBDIR}/abcdef0`);
  });
});
...
describe("exportReplaySnapshot (#3010)", () => {
  it("rejects a path-traversal commitSha before running any git command (#7796)", async () => {
    const { exec, calls } = scriptedExec([]); // no scripts -- any git call would throw "no script matched"
    const store = tempStore();

    await expect(
      exportReplaySnapshot({ repoPath: "/repo", repoFullName: "acme/widgets", commitSha: "../../../tmp/evil" }, { exec, store }),
    ).rejects.toThrow("invalid_commit_sha");
    expect(calls).toHaveLength(0);
    expect(store.getSnapshot("acme/widgets", "0000000")).toBeNull();
  });
  ...
  it("a commit at the very first commit of history: git log returns exactly one entry, no parents to walk", async () => {
    const { exec } = scriptedExec(
      happyPathScripts([
        { match: isHistory, result: ok(`0000007${FIELD_SEP}2020-01-01T00:00:00+00:00${FIELD_SEP}initial commit\n`) },
    ...
    const snapshot = await exportReplaySnapshot({ repoPath: "/repo", repoFullName: "acme/widgets", commitSha: "0000007" }, { exec, store });

    expect(snapshot.commits).toEqual([{ sha: "0000007", date: "2020-01-01T00:00:00+00:00", subject: "initial commit" }]);
  });
