  ...
  it("counts matching rows across the twelve real stores without writing anything, and reports attempt-log as not-purgeable", async () => {
    ...
    expect(result).toMatchObject({
      ...
      stores: [
        ...
        // worktree-allocator's file is never created in this fixture (no attempt was ever acquired), so its
        // dry-run count is 0 -- a dedicated test below seeds a real slot pool and asserts a non-zero count.
...
describe("runPurge (real, #5564, #6599)", () => {
  ...
  it("REGRESSION (#8320): worktree-allocator dry-run count and real purge agree, counting only a stale FREE slot and never a live active one", () => {
    const root = tempDir();
    const worktreeDbPath = join(root, "worktree-allocator.sqlite3");
    const allocator = openWorktreeAllocator({
      dbPath: worktreeDbPath,
      worktreeBaseDir: join(root, "worktrees"),
      maxConcurrency: 2,
    });
    closeables.push(allocator);
    // slot 0: a live, in-flight attempt for the target repo — its repo_full_name reflects a real on-disk
    // checkout, so it must survive the purge untouched and never be counted as purgeable.
    ...
    // blanks repo_full_name to NULL on free, so this is seeded directly via a separate connection.
    const seed = new DatabaseSync(worktreeDbPath);
    seed.exec("UPDATE worktree_slots SET repo_full_name = 'acme/widgets' WHERE slot_index = 1");
    seed.close();

    // Every other store resolves to a nonexistent temp path (count 0); only worktree-allocator has a real file.
    const resolveDbPaths = {
      "claim-ledger": () => join(root, "claim-ledger.sqlite3"),
      "event-ledger": () => join(root, "event-ledger.sqlite3"),
      "governor-ledger": () => join(root, "governor-ledger.sqlite3"),
      ...
      "replay-snapshot": () => join(root, "replay-snapshot.sqlite3"),
      "deny-hook-synthesis": () => join(root, "deny-hook-synthesis.sqlite3"),
      "worktree-allocator": () => worktreeDbPath,
      "attempt-log": () => join(root, "attempt-log.sqlite3"),
    };
    ...
    expect(runPurge(["--repo", "acme/widgets", "--dry-run", "--json"], { resolveDbPaths })).toBe(0);
    const dryRun = JSON.parse(String(log.mock.calls[0]?.[0]));
    // The dry run previews exactly the one stale free slot; the active slot is never purgeable.
    expect(dryRun.stores).toContainEqual({ store: "worktree-allocator", wouldPurge: 1 });

    // The real purge removes exactly that count and leaves the live active slot fully intact.
    ...
    expect(slots.find((slot) => slot.slotIndex === 1)).toMatchObject({ status: "free", repoFullName: null });

    // Idempotent: only the active slot still names the repo, so a second purge clears (and counts) nothing.
