...
beforeAll(async () => {
  tempDir = mkdtempSync(join(tmpdir(), "loopover-automation-state-"));
...
describe("bin loopover_get_automation_state stdio tool (in-process, #7752)", () => {
  it.each(MODULES)("registers and proxies GET .../automation-state — %s", async (specifier) => {
    ...
    const client = new Client({ name: "automation-state-test", version: "0.1.0" }, { capabilities: {} });
    await client.connect(clientTransport);
    try {
      const { tools } = await client.listTools();
      const tool = tools.find((entry) => entry.name === "loopover_get_automation_state");
      expect(tool).toBeDefined();
      expect(tool?.description).toMatch(/automation state/i);
      ...
      expect(text).toContain("actingActionClasses");
    } finally {
